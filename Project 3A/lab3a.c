...
#define SINGLY_IND_BLOCKS 256
#define DOUBLY_IND_BLOCKS 65536
#define TRIPLY_IND_BLOCKS 16777216
...
__u32 calculate_block_num(int *level, __u32 *block, __u32 global_block_num) {
    ...
    const __u32 offsets[4] = {12, my_block_size+12, my_block_size*my_block_size + my_block_size + 12, my_block_size*my_block_size*my_block_size + my_block_size*my_block_size + my_block_size + 12};
...
void directory_entries(){
    ...
    for (int i = 0; i < group_count; i++) {
        ...
        __u32 offset = inode_table_block_id * block_size;
        __u32 num_inodes_in_group = groupSummary[i].inodes_in_group;
        //    printf("num_inodes: %u", num_inodes_in_group);
...
//                                printf("breakout\n");
                            ...
                            fprintf(my_csv, "DIRENT,%u,%u,%u,%u,%u,%s\n", parent_inode, local_offset,
                                    referenced_inode, rec_len, name_len, curr_dir_entry.name);
...
void indirected_block_reference(){
  ...
  for (int i = 0; i < group_count; i++) {
	...
	if(curr_byte & mask){
	  ...
	  int indirect_level = 1;
	  int global_block_number = 13;
	  int local_block_number = 1;
	  uint16_t i_mode = curr_inode.i_mode;
	  uint32_t i_block[15];
	  for(int c = 0; c < 15; c++){
	    i_block[c] = curr_inode.i_block[c];
	  ...
	  if( i_mode & 0x4000 || i_mode & ){
	    while(1){
	      if(indirect_level == 1){
		if(local_block_number > 256)
		  indirect_level++;
